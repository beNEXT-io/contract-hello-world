...
```bash
shubham-rust-sam-app$ sam build
```
...
```bash
shubham-rust-sam-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
shubham-rust-sam-app$ sam local start-api
shubham-rust-sam-app$ curl http://localhost:3000/
```
...
```bash
shubham-rust-sam-app$ sam logs -n HelloWorldFunction --stack-name shubham-rust-sam-app --tail
```
